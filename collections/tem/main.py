...
user_agent = {'User-Agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/78.0.3904.70 Safari/537.36'}
...
def login(email, password): # 本函数copy自https://gist.github.com/fyears/487fc702ba814f0da367a17a2379e8ba，感谢@fyears
    ...
    while True:
        try:
            temp = client.get(sign_in_url, verify=False)
            print('hereddd')
...
def scraping(client):
    ...
    while True:
        random.seed(10)
        time.sleep(random.random()*3 + 0.5) #速度稍快就会被限制
                ...
                if Pid == 0 or Title in invalidset:
                    ...
                    if Title not in invalidset: #第一次没找到
                        with open("Log.txt", "a") as log:
                            log.write("Unknown PID happened for {}\n".format(Title))
